...
rc.8 Release、标签及同版 ZIP 已公开，按以下固定版本步骤安装。
...
本版已发布；离线回归、完整包校验与实际插件安装分别核对。异机真实微信／小红书 smoke 与第二用户 UAT 仍需每位使用者在自己的电脑和账号上完成。发现问题时请同时提供版本号或提交号、使用渠道、可复现步骤和停止位置；不要提交账号态、查询隐私、客户数据或截图中的敏感信息。
