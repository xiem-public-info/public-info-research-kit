...
本版移植公开网页发现与缺口补证、上下游证据反馈、共享GUI提交前文字核对、社媒停止条件及建筑素材边界。实际执行绑定与住宅接口沿用rc.7；本版为rc.8全量包，旧版rc.7和附件保留。
...
rc.8 保留此前渠道能力、原研究标准及实际执行／后续授权绑定，补齐近期可迁移方法。更新内容见 [本版说明](RELEASE_NOTES_0.8.0-rc.8.md)。可复制 [安装提示词](INSTALL_PROMPT_0.8.0-rc.8.md)，首次安装见 [中文说明](RC_INSTALL_0.8.0.md)，已有安装见 [升级与回退](UPGRADE_0.8.0-rc.7_to_rc.8.md)。需要回退时可使用 [rc.7历史版本](https://github.com/xiem-public-info/public-info-research-kit/releases/tag/v0.8.0-rc.7)，旧标签和历史成果保留。

包内清单记录构建核验时的状态；是否已经公开发布以同一固定标签的 Release 为准。

本次保留任务级授权和现行渠道操作，支持已知链接直接读取、公开机构报告发现、空间部分交付、按缺口选图及按业务启用住宅研究；充分性按累计有效成果判断，直接读取不豁免原任务明确的研究要求。离线检查不代表异机真实检索或业务验收。
...
0.8.0-rc.8 保留研究型搜索策略和微信 `WECHAT-LITE-DIRECT-CU-V1` Stable 路线：使用者首次登录并打开公开“搜一搜”后，Codex 通过 Computer Use 直接完成一次查询输入、一次提交、开文、逐次正文推进和安全关页。该路线已在研发基线完成跨批次真实链路验证；发行包不包含旧 V2、Pad 或 AX 路线。
