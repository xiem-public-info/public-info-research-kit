...
当前分支在已发布 `v0.8.0-rc.3` 基础上收录 2026-09-07 全渠道规则校准，属于尚未单独发版的候选修订。已发布标签与稳定 `main / v0.7.0` 不变；旧标签下载不包含本次修订。当前候选分支与提交才是本次代码口径。[原 rc.3 发布记录](https://github.com/xiem-public-info/public-info-research-kit/releases/tag/v0.8.0-rc.3)；[中文安装说明](RC_INSTALL_0.8.0.md)。

本次统一任务授权、已知微信链接读取、公开机构报告发现、按对象部分交付、按缺口选图和按业务启用住宅研究段，修正地图与动态页的桌面调度。未发布新的 tag 或 Release，不代表异机真实检索或用户验收完成。
...
0.8.0-rc.3 保留研究型搜索策略和微信 `WECHAT-LITE-DIRECT-CU-V1` Stable 路线：使用者首次登录并打开公开“搜一搜”后，Codex 通过 Computer Use 直接完成一次查询输入、一次提交、开文、逐次正文推进和安全关页。该路线已在研发基线完成跨批次真实链路验证；发行包不包含旧 V2、Pad 或 AX 路线。
