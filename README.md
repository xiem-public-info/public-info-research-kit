...
本版同步模型理解与计划校验分工、交付与尾盘判断、小红书原生入口。公开 main 与固定版本 v0.8.0-rc.5 同步；本版仍为预发布。
...
当前公开 main 与固定版本 `v0.8.0-rc.5` 收录本轮范围职责、计划校验与渠道入口修正，并保留前版的直接读取适用性修正。旧版本标签保持不变；需要固定版本时请从 [rc.5 发布页](https://github.com/xiem-public-info/public-info-research-kit/releases/tag/v0.8.0-rc.5) 下载。可直接复制 [安装部署提示词](INSTALL_PROMPT_0.8.0-rc.5.md)，更新内容见 [本版发布说明](RELEASE_NOTES_0.8.0-rc.5.md)，步骤见 [中文安装说明](RC_INSTALL_0.8.0.md)。
...
0.8.0-rc.5 保留研究型搜索策略和微信 `WECHAT-LITE-DIRECT-CU-V1` Stable 路线：使用者首次登录并打开公开“搜一搜”后，Codex 通过 Computer Use 直接完成一次查询输入、一次提交、开文、逐次正文推进和安全关页。该路线已在研发基线完成跨批次真实链路验证；发行包不包含旧 V2、Pad 或 AX 路线。
