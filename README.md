# 公开信息研究工具包 0.8.0-rc.7

本版补齐原生与住宅研究的实际执行请求检查、充分性回传绑定和旧回包只读兼容，保持住宅公开 rc.8 的接口不变。本版为预发布，是否已公开以固定 v0.8.0-rc.7 Release 为准；固定标签与资产未公开时继续使用现有版本。
...
rc.7 保留此前渠道能力和原研究标准，新增实际执行检查与独立后续授权绑定。更新内容见 [本版说明](RELEASE_NOTES_0.8.0-rc.7.md)。发布后可复制 [安装提示词](INSTALL_PROMPT_0.8.0-rc.7.md)，首次安装见 [中文说明](RC_INSTALL_0.8.0.md)，已有安装见 [升级与回退](UPGRADE_0.8.0-rc.6_to_rc.7.md)。固定 rc.7 尚未公开时可使用 [rc.6](https://github.com/xiem-public-info/public-info-research-kit/releases/tag/v0.8.0-rc.6)，旧标签和历史成果保留。
...
0.8.0-rc.7 保留研究型搜索策略和微信 `WECHAT-LITE-DIRECT-CU-V1` Stable 路线：使用者首次登录并打开公开“搜一搜”后，Codex 通过 Computer Use 直接完成一次查询输入、一次提交、开文、逐次正文推进和安全关页。该路线已在研发基线完成跨批次真实链路验证；发行包不包含旧 V2、Pad 或 AX 路线。
