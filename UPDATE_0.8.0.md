...
0.8.0-rc.5 是“质量与行为同构”候选：它把研发主线中可公开、可迁移的渠道画像、检索方法、充分性、自适应增量、证据拒收、黄金样例和回归 harness 带入公开包，同时继续排除账号态、真实业务样本、内部运行状态和退役路线。

本次 rc.5 收录模型理解与计划校验分工、明确排除与业务领域、复用和暂缓不触发检索、临近交付与尾盘判断、小红书原生入口修正。保留 rc.4 的累计有效成果充分性及直接读取不能豁免原研究要求。公开 main 同步本版本，旧标签不改写。详见 [本版更新](RELEASE_NOTES_0.8.0-rc.5.md) 与 [安装部署提示词](INSTALL_PROMPT_0.8.0-rc.5.md)。
